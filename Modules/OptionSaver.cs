...
{
    private static readonly DirectoryInfo SaveDataDirectoryInfo = new("./TOC_DATA/SaveData/");
    private static readonly FileInfo OptionSaverFileInfo = new($"{SaveDataDirectoryInfo.FullName}/Options.json");
    private static readonly LogHandler Logger = TOC.Logger.Handler(nameof(OptionSaver));
    ...
    private static SerializableOptionsData GenerateOptionsData()
    {
        ...
        foreach (OptionItem option in OptionItem.AllOptions)
        {
            if (option.IsSingleValue)
            {
                ...
                {
                    TOC.Logger.Warn($"Duplicate SingleOption ID: {option.Id}", "Options Load");
                }
            }
            else if (!presetOptions.TryAdd(option.Id, option.AllValues))
            {
                TOC.Logger.Warn($"Duplicate preset option ID: {option.Id}", "Options Load");
            }
        }
